...
# specific to toolbox
from MarkovModel import IdealObserver as IO
from MarkovModel import GenerateSequence as sg

# %% Binary sequence and order 0 transition probabilities

# Generate sequence
L = int(1e2)
seq = sg.ProbToSequence_Nitem2_Order0(
...
plt.subplot(4, 1, 2)
vmin, vmax = 0, np.max([np.max(out_hmm[(0, 0)]['dist']),
                        np.max(out_hmm[(1, 0)]['dist'])])
...
plt.subplot(4, 1, 4)
plt.plot(out_hmm[(0, 0,)]['mean'], label='p(0|0)')
plt.plot(out_hmm[(1, 0,)]['mean'], label='p(0|1)')
plt.legend(loc='best')
plt.title('mean hmm')
plt.ylim([0, 1])
plt.show()
...
# %% Sequence of 3 items and order 0 vs order 1 transition probabilities
...
options = {'Decay': 10, 'p_c': 1/200, 'resol': 20}
out_fixed = IO.IdealObserver(seq, 'fixed', order=0, options=options)
out_fixed1 = IO.IdealObserver(seq, 'fixed', order=1, options=options)
...
plt.ylim([0, 1])
plt.subplot(2, 1, 2)
plt.plot(out_fixed1[(0, 0)]['mean'], label='p(0|0)')
plt.plot(out_fixed1[(1, 0)]['mean'], label='p(0|1)')
plt.plot(out_fixed1[(2, 0)]['mean'], label='p(0|2)')
plt.legend(loc='best')
plt.title('mean')
plt.ylim([0, 1])
...
grid_nu = 1/2 ** np.array([k/2 for k in range(20)])
options = {'resol': 20, 'grid_nu': grid_nu, 'prior_nu': np.ones(20)/20}
out_hmm_unc_full = IO.IdealObserver(
    seq, 'hmm_uncoupled+full', order=1, options=options)
out_hmm_full = IO.IdealObserver(seq, 'hmm+full', order=1, options=options)
...
plt.yticks(ticks=[0, options['resol']/2, options['resol']], labels=[0, 0.5, 1])
plt.title('p(0|0), full inference (unc)', **
...
plt.imshow(out_hmm_full[(0, 0)]['dist'], origin='lower')
plt.yticks(ticks=[0, options['resol']/2, options['resol']], labels=[0, 0.5, 1])
plt.title('p(0|0), full inference (coupled)', **
          {'fontname': 'Arial', 'size': '12'})
plt.tick_params(axis='x', which='both', bottom=False,
                top=False, labelbottom=False)
plt.tight_layout()
...
# Compute Decay observer and HMM
options = {'Decay': 4, 'p_c': 1/200, 'resol': 20}
out_fixed = IO.IdealObserver(seq, 'fixed', order=1, options=options)
out_hmm = IO.IdealObserver(seq, 'hmm', order=1, options=options)

# %%
# Test
seq = np.array([1, 1, 2, 1, 1, 2, 1, 2, 1, 1, 1, 2]) - 1
...
# In the HMM observer, p_c is the a priori probability of a change point on each trial
# For HMM, 'resol' is the number on bins used for numeric estimation of integrals on a grid.
options = {'Decay': 4, 'p_c': 1/10, 'resol': 5}
out_fixed = IO.IdealObserver(seq, 'fixed', order=1, options=options)
out_hmm = IO.IdealObserver(seq, 'hmm', order=1, options=options)
